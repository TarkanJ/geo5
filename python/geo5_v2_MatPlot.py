...
def plot_points():
    if not points:
        messagebox.showwarning("Pozor", "Žádné body")
        return
    ...
    for p in points:
        xs.append(p.x)
        ys.append(p.y)
        cs.append(colors.get(p.typ.lower(), "green"))

    plt.figure()
    plt.scatter(xs, ys, c=cs)

    for p in points:
        plt.text(p.x, p.y, str(p.id), fontsize=8)

    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title("Geodetické body")
...
tk.Button(frame_top, text="Zobrazit mapu", command=plot_points).grid(row=0, column=2, padx=5)
